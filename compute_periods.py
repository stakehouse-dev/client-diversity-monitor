...
DEFAULT_BN = cfg.get_beacon_node_url()
BEACON_NODE_HEADER = {"x-api-key": cfg.beacon_node_api_key()}
...
def get_head_slot(bn_url):
    res = requests.get(f"{bn_url}/eth/v1/beacon/headers/head", headers=BEACON_NODE_HEADER)
...
def get_active_validator_count(slot, bn_url):
    res = requests.get(f"{bn_url}/eth/v1/beacon/states/{slot}/validators", headers=BEACON_NODE_HEADER)
    res.raise_for_status()
